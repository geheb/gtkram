...
public sealed class SellerRegistrations : IDisposable
{
    ...
    public async Task<BazaarSellerRegistrationDto[]> GetAll(Guid eventId, CancellationToken cancellationToken)
    {
        ...
        return entities.Select(e => new BazaarSellerRegistrationDto(e.reg, e.count, idn)).ToArray();
    }
    ...
    public async Task<bool> NotifyRegistration(Guid registrationId, CancellationToken cancellationToken)
    {
        ...
        if (!hasPendingNotification)
        {
            ...
            {
                Id = _pkGenerator.Generate(),
                Type = (int)AccountEmailTemplate.RegisterSeller,
