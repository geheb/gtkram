...
internal sealed class MysqlMigration
{
    ...
    private readonly ISqlRepository<Models.Event> _eventRepository;
    private readonly ISqlRepository<Models.Identity> _identityRepository;
    private readonly ISqlRepository<Models.SellerRegistration> _sellerRegistationRepository;
    private readonly ISqlRepository<Models.Seller> _sellerRepository;
    private readonly ISqlRepository<Models.Article> _articleRepository;
    private readonly ISqlRepository<Models.Checkout> _checkoutRepository;

    public MysqlMigration(
        IConfiguration configuration,
        SQLiteDbContext sqlite,
        ISqlRepository<Models.Event> eventRepository,
        ISqlRepository<Models.Identity> identityRepository,
        ISqlRepository<Models.SellerRegistration> sellerRegistationRepository,
        ISqlRepository<Models.Article> articleRepository,
        ISqlRepository<Models.Checkout> checkoutRepository,
        ISqlRepository<Models.Seller> sellerRepository)
    {
    ...
    private async Task InsertCheckouts(DbConnection connection, CancellationToken cancellationToken)
    {
        ...
        foreach (var e in checkouts)
        {
            ...
            var checkout = new Models.Checkout
            {
                Id = new Guid(id),
    ...
    private async Task InsertIdentities(DbConnection mysql, CancellationToken cancellationToken)
    {
        ...
        foreach (var e in users)
        {
            ...
            var claims = new List<Models.IdentityClaim>();
            ...
            {
                claims.Add(new Models.IdentityClaim(ClaimTypes.Role, role == "billing" ? Roles.Checkout : role));
            }

            var tfa = await mysql.QueryFirstOrDefaultAsync<string?>("select ClaimValue from user_claims where UserId=@id", new { id });
            if (tfa == "1")
            {
                claims.Add(new Models.IdentityClaim(UserClaims.TwoFactorClaim));
            }
    ...
    public async Task InsertEvents(DbConnection connection, CancellationToken cancellationToken)
    {
        ...
        foreach(var e in events)
        {
            ...
            await _eventRepository.Insert(new Models.Event()
            {
                Id = new Guid(id),
                Created = (DateTime)e.CreatedOn,
                Json = new()
