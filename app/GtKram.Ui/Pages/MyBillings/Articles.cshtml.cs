...
public class ArticlesModel : PageModel
{
    ...
    {
        var result = await _mediator.Send(new GetBillingArticlesWithBillingAndEventByUserQuery(User.GetId(), id), cancellationToken);
        if (result.IsFailed)
        {
    ...
    {
        var result = await _mediator.Send(new CreateBillingArticleByUserCommand(User.GetId(), id, articleId), cancellationToken);
        if (result.IsSuccess)
        {
    ...
    {
        var result = await _mediator.Send(new DeleteBillingArticleByUserCommand(User.GetId(), id), cancellationToken);
    ...
    {
        var result = await _mediator.Send(new CancelBillingByUserCommand(User.GetId(), id), cancellationToken);
        return new JsonResult(result.IsSuccess);
    }

    public async Task<IActionResult> OnPostCompleteAsync(Guid id, CancellationToken cancellationToken)
    {
        var result = await _mediator.Send(new CompleteBillingByUserCommand(User.GetId(), id), cancellationToken);
        return new JsonResult(result.IsSuccess);
    }
}
